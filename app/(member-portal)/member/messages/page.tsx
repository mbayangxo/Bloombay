...
// ── Mailbox Items (permanent/meaningful things) ───────────────────────────────

const MAILBOX_ITEMS: MailboxItem[] = [
  {
    id: 100, type: "letter", from: "Yande",
    initial: "Y", color: "#FF1F7D",
  ...
  {
    id: 101, type: "certificate", from: "BloomBay",
    initial: "✦", color: "#D4A853",
  ...
  {
    id: 102, type: "invitation", from: "African Girls Club",
    initial: "AG", color: "#FF69B4",
  ...
  {
    id: 103, type: "milestone", from: "BloomBay",
    initial: "✦", color: "#FF1F7D",
  ...
  {
    id: 104, type: "recognition", from: "BloomBay",
    initial: "✦", color: "#D4A853",
...
// ── MailboxItemCard — compact row for mailbox items ──────────────────────────
    ...
    <button onClick={onClick}
      ...
      <div className="relative flex-shrink-0">
        <div className="w-11 h-11 rounded-full flex items-center justify-center font-bold text-white flex-shrink-0"
          style={{
            background: item.type === "certificate" || item.type === "recognition"
              ? "linear-gradient(135deg, #D4A853, #B8922D)"
              : `linear-gradient(135deg, ${item.color}, ${item.color}BB)`,
            fontSize: item.initial.length > 1 ? "10px" : "16px",
            boxShadow: isUnopened ? `0 0 0 2px ${item.color}, 0 0 0 4px var(--pale-pink-bg)` : "none",
          }}>
          {item.initial}
        </div>
        {isUnopened && (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-0.5">
          <p className="font-bold text-sm truncate leading-tight"
            style={{ color: "var(--heading-color, #111111)", fontWeight: isUnopened ? 700 : 500 }}>
            {item.subject}
          </p>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <p className="text-xs truncate" style={{ color: "var(--text-muted, #bbb)" }}>{item.preview}</p>
        </div>
      </div>
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(180,140,140,0.5)" strokeWidth="2.5" strokeLinecap="round" className="flex-shrink-0">
        <polyline points="9 18 15 12 9 6"/>
...
// ── Letter View — opens a mailbox item ───────────────────────────────────────
...
function LetterView({ item, onBack }: { item: MailboxItem; onBack: () => void }) {
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: isGold ? "#0A0804" : "var(--pale-pink-bg)" }}>
      ...
      {/* Letter / Certificate card */}
      <div className="px-5 md:px-8">
        <div className="rounded-3xl overflow-hidden relative"
          style={{
            background: isGold
              ? "linear-gradient(145deg, #1A1208 0%, #0A0804 100%)"
              : "white",
            boxShadow: isGold
              ? "0 16px 48px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(212,168,83,0.2)"
          ...
          <div className="px-6 pt-6 pb-3">
            <div className="flex items-start gap-4 mb-5">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[9px] font-bold tracking-[0.22em] uppercase mb-1" style={{ color: `${accentColor}99` }}>FROM {item.from.toUpperCase()}</p>
...
type View = "hub" | "thread" | "plan" | "letter";
...
export default function MessagesPage() {
  ...
  function back() { setView("hub"); setActiveChat(null); setActiveItem(null); }
  ...
  const MAILBOX_FILTERS: { label: string; value: MailboxItemType | "all" }[] = [
    { label: "All", value: "all" },
  ...
  const totalUnread = MAILBOX_ITEMS.filter(i => !openedItems.has(i.id)).length;
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: "var(--pale-pink-bg)" }}>
      ...
      {/* Mailbox list — permanent & beautiful items */}
      <div className="mx-5 md:mx-8 rounded-3xl overflow-hidden" style={{ background: "var(--card-bg, white)", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
        {shown.length === 0 ? (
          <div className="py-16 text-center px-6">
            ...
            <p className="text-sm italic" style={{ fontFamily: "var(--font-instrument)", color: "var(--text-muted, #bbb)" }}>
      ...
      <div className="px-5 mt-4 md:px-8">
        <Link href="/member/chat"
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(180,140,140,0.5)" strokeWidth="2.5" strokeLinecap="round">
            <polyline points="9 18 15 12 9 6"/>
          </svg>
        </Link>
      </div>
