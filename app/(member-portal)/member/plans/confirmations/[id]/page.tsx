...
function EventTicket({ data, qrUrl }: { data: ConfirmationData; qrUrl: string }) {
  ...
  return (
    <div style={{ borderRadius: 22, overflow: "hidden", background: "var(--bb-card)", boxShadow: "0 24px 72px rgba(0,0,0,0.22)", border: "1px solid var(--bb-border-strong)" }}>
      {/* Poster strip */}
      <div style={{ height: 200, position: "relative", overflow: "hidden", background: hasPoster ? "transparent" : `linear-gradient(155deg, #18080F 0%, #3A0020 45%, ${PINK} 100%)` }}>
        {hasPoster && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={data.event.poster!} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        )}
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(0,0,0,0.15) 0%, rgba(0,0,0,0.0) 30%, rgba(0,0,0,0.75) 100%)" }} />
        <div style={{ position: "absolute", top: 16, left: 16, display: "flex", alignItems: "center", gap: 6, background: "rgba(0,0,0,0.38)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 999, padding: "5px 10px", border: "1px solid rgba(255,255,255,0.15)" }}>
        ...
        <div style={{ display: "flex", gap: 14, marginBottom: 18, alignItems: "flex-start" }}>
          <div style={{ flex: 1 }}>
            <div style={{ marginBottom: 12 }}>
              <p style={{ fontFamily: "var(--font-jost)", fontSize: "7px", fontWeight: 800, letterSpacing: "0.14em", color: "var(--bb-text-muted)", marginBottom: 4 }}>DATE & TIME</p>
            ...
            {data.event.venue && (
              <div style={{ marginBottom: 12 }}>
                <p style={{ fontFamily: "var(--font-jost)", fontSize: "7px", fontWeight: 800, letterSpacing: "0.14em", color: "var(--bb-text-muted)", marginBottom: 4 }}>VENUE</p>
                <p style={{ fontFamily: "var(--font-jost)", fontSize: 12, color: "var(--bb-text-2)", lineHeight: 1.4 }}>{data.event.venue}</p>
              </div>
            )}
            <div>
              <p style={{ fontFamily: "var(--font-jost)", fontSize: "7px", fontWeight: 800, letterSpacing: "0.14em", color: "var(--bb-text-muted)", marginBottom: 4 }}>CONFIRMATION CODE</p>
    ...
    <div style={{ minHeight: "100vh", background: "var(--bb-bg)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ position: "relative", height: 230, overflow: "hidden", background: hasPoster ? "transparent" : `linear-gradient(145deg, #18080F 0%, #5A0030 55%, ${PINK} 100%)` }}>
        ...
        <div style={{ position: "absolute", bottom: 22, left: 20, right: 20 }}>
          ...
          {data.event.venue && <p style={{ fontFamily: "var(--font-jost)", fontSize: 11, color: "rgba(255,255,255,0.6)", marginTop: 3 }}>{data.event.venue}</p>}
        </div>
      </div>
      ...
      <div style={{ flex: 1, background: "var(--bb-bg)", borderRadius: "22px 22px 0 0", marginTop: -18, position: "relative", zIndex: 2, padding: "24px 20px calc(env(safe-area-inset-bottom, 0px) + 100px)", animation: "fadeUp 0.42s ease" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 13, marginBottom: 20 }}>
          ...
          <div>
            <h1 style={{ fontFamily: "var(--font-playfair)", fontStyle: "italic", fontWeight: 900, fontSize: 32, color: "var(--bb-text)", lineHeight: 0.92, margin: 0 }}>You&apos;re in.</h1>
            {data.confirmed_at && <p style={{ fontFamily: "var(--font-jost)", fontSize: 11, color: "var(--bb-text-muted)", marginTop: 4 }}>Confirmed {formatShortDate(data.confirmed_at)}</p>}
          </div>
        </div>

        {/* Event detail card */}
        <div style={{ background: "var(--bb-card)", borderRadius: 16, border: "1px solid var(--bb-border)", padding: "14px 16px", marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontFamily: "var(--font-jost)", fontSize: 11, color: "var(--bb-text-3)" }}>
              {formatTime(data.event.starts_at)}{data.attendee_count > 1 ? ` · ${data.attendee_count} confirmed` : ""}
            </p>
          </div>
          <div style={{ background: `${PINK}12`, border: `1px solid ${PINK}28`, borderRadius: 8, padding: "5px 10px" }}>
            <p style={{ fontFamily: "var(--font-jost)", fontSize: "8px", fontWeight: 800, color: PINK }}>IN ✓</p>
          </div>
        </div>

        {/* Ticket icon button → view ticket */}
        <button
          onClick={() => setStep("ticket")}
          style={{ width: "100%", marginBottom: 10, padding: "16px 18px", borderRadius: 18, background: `linear-gradient(135deg, ${PINK}, #c4005a)`, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 14, textAlign: "left", boxShadow: `0 6px 24px rgba(255,31,125,0.32)` }}
        >
          {/* Ticket stub icon */}
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontFamily: "var(--font-jost)", fontWeight: 800, fontSize: 14, color: "white", marginBottom: 2 }}>View your ticket</p>
        ...
        {/* All confirmations icon link */}
        <Link href="/member/plans/confirmations" style={{ textDecoration: "none", display: "block", marginBottom: 10 }}>
          <div style={{ padding: "14px 18px", borderRadius: 18, background: "var(--bb-card)", border: "1px solid var(--bb-border-strong)", display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 10, background: `${PINK}10`, border: `1px solid ${PINK}22`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              {/* Stack of tickets icon */}
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={PINK} strokeWidth="1.8" strokeLinecap="round">
                <rect x="3" y="8" width="18" height="12" rx="2"/>
                <path d="M3 12h18" strokeDasharray="2 2"/>
                <path d="M7 4h10" opacity="0.5"/>
                <path d="M5 6h14" opacity="0.3"/>
              </svg>
            </div>
            <div style={{ flex: 1 }}>
              <p style={{ fontFamily: "var(--font-jost)", fontWeight: 700, fontSize: 13, color: "var(--bb-text)", marginBottom: 2 }}>All your confirmations</p>
              <p style={{ fontFamily: "var(--font-jost)", fontSize: 11, color: "var(--bb-text-3)" }}>Past and upcoming · in one place</p>
            </div>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontFamily: "var(--font-jost)", fontWeight: 700, fontSize: 13, color: "var(--bb-text)", marginBottom: 2 }}>Plan room</p>
