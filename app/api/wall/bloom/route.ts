...
// Body: { postId: string } — server derives current state from DB (never trust client)
export async function POST(req: NextRequest) {
  ...
  // Derive actual state from DB — never trust client-supplied bloomed boolean
  ...
  return NextResponse.json({ blooms: data?.blooms ?? 0, bloomed: !existing });
}
