...
export async function POST(req: NextRequest) {
  const guard = cronGuard(req, "club-success");
  ...
  try {
    const { data: clubs, error: clubErr } = await supabase
      ...
      .limit(cronMaxRecords(30));

    if (clubErr || !clubs?.length) {
      await logCronRun("club-success", "skipped", { reason: "no clubs", error: clubErr?.message });
    ...
    for (const club of clubs) {
      try {
        ...
        const { count: recentEvents } = await supabase
          ...
          .or(`creator_user_id.eq.${club.owner_id},host_id.eq.${club.owner_id}`)
          .gte("starts_at", daysAgo30);
...
Gatherings hosted last 30 days: ${recentEvents ?? 0}
`.trim();
...
Focus on the most important signal only. Sound like a smart friend, not a dashboard.`,
    ...
    await logCronRun("club-success", "ok", { records_processed: notified, clubs_scanned: clubs.length });
    return NextResponse.json({ ok: true, clubs: clubs.length, notified });
  } catch (err) {
    await logCronRun("club-success", "error", { error: String(err) });
