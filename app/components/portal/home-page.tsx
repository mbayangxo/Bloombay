...
function EventPoster({ ev }: { ev: typeof CITY_EVENTS[0] }) {
  return (
    <Link href="/member/happenings" style={{ textDecoration: "none" }}>
      <div className="rounded-2xl overflow-hidden relative" style={{ height: "170px", background: ev.gradient }}>
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: "radial-gradient(ellipse at 80% 20%, rgba(255,255,255,0.08) 0%, transparent 55%)" }} />
        <div className="absolute inset-0 p-4 flex flex-col justify-between">
          <div>
            <p className="text-[8px] font-bold tracking-[0.22em] uppercase"
              style={{ color: "rgba(255,255,255,0.55)" }}>
              {ev.label}
            </p>
          </div>
          <div>
            <p className="font-bold leading-none mb-1 text-white"
              style={{ fontFamily: "var(--font-playfair)", fontSize: "1.15rem", lineHeight: 1.1 }}>
              {ev.title}
            </p>
            <p className="text-[11px] mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>{ev.sub}</p>
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-bold" style={{ color: "rgba(255,255,255,0.35)" }}>{ev.venue}</p>
              <span className="text-[10px] font-bold px-2 py-1 rounded-full"
                style={{ background: "rgba(255,255,255,0.18)", color: "white" }}>{ev.seats}</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="flex flex-col gap-3">
              {INVITATIONS.map((inv) => (
                <Link key={inv.id} href="/member/happenings"
                  ...
                  style={{
                    background: cardBg,
                    boxShadow: isNight ? "0 4px 20px rgba(0,0,0,0.2)" : "0 2px 12px rgba(255,31,125,0.08)",
                    minHeight: "88px",
                    border: isNight ? "1px solid rgba(255,255,255,0.05)" : "1px solid rgba(255,31,125,0.08)",
                  }}>
                  {/* Ticket stub */}
                  <div className="w-14 flex-shrink-0 flex flex-col items-center justify-center relative"
                    style={{ background: "#FF1F7D", borderRight: "2px dashed rgba(255,255,255,0.3)" }}>
                  ...
                  <div className="flex-1 px-4 py-3.5">
                    <p className="text-[10px] font-bold tracking-wider uppercase mb-1" style={{ color: "#FF1F7D" }}>
                      {inv.from}
                    </p>
                    <p className="font-bold text-sm leading-snug" style={{ color: headingColor }}>{inv.title}</p>
                    <p className="text-xs mt-0.5" style={{ color: textMuted }}>{inv.detail}</p>
                  </div>
