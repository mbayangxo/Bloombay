...
import { ClubLayoutHero } from "./club-layout-hero";
...
export function ClubLandingPage({ club = DEFAULT_CLUB, isMember = false }: { club?: ClubLandingData; isMember?: boolean }) {
  ...
  const layout = club.layoutKey ?? "salon";
  const accent = club.accentColor ?? club.color;
  const pageBg = club.darkBg ? "#0F0F0F" : "var(--pale-pink-bg)";
  const headingColor = club.darkBg ? "rgba(255,238,220,0.92)" : "#111111";
  const cardBg = club.darkBg ? "#1A1410" : "white";
  const textMuted = club.darkBg ? "rgba(215,175,155,0.58)" : "#888";
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: pageBg }}>
      ...
      <ClubLayoutHero
        ...
        color={club.color}
        accentColor={accent}
        crestBg={club.crestBg ?? "#3a0018"}
        coverUrl={club.coverUrl ?? club.photos?.[0]}
        darkBg={club.darkBg}
        crestSymbol={club.crestSymbol}
        crestAccent={club.crestAccent}
        logoUrl={club.logoUrl}
        logoTemplate={club.logoTemplate}
      ...
      {!isMember && (
        <div
          ...
          style={{
            background: club.darkBg ? "rgba(15,15,15,0.92)" : "rgba(255,245,248,0.95)",
            backdropFilter: "blur(12px)",
            borderBottom: club.darkBg ? "1px solid rgba(255,255,255,0.08)" : "1px solid #F0D0DC",
          }}
        >
      ...
      {(!isMember || clubTab === "about") && (
        <div className="max-w-xl mx-auto px-5 pt-6 flex flex-col gap-6">
          ...
          {/* Tagline — salon layout only (editorial/midnight show in hero) */}
          {layout === "salon" ? (
            <p
          ...
          {/* About */}
          <div className="rounded-3xl p-5" style={{ background: cardBg, boxShadow: club.darkBg ? "none" : "0 1px 8px rgba(0,0,0,0.05)", border: club.darkBg ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
            <p className="text-xs font-bold tracking-widest uppercase mb-3" style={{ color: club.color }}>ABOUT THIS CLUB</p>
            <p className="text-sm leading-relaxed" style={{ color: textMuted }}>{club.about}</p>
          </div>
          ...
          <div className="rounded-3xl p-5" style={{ background: `${club.color}12`, border: `1px solid ${club.color}30` }}>
            ...
            <p className="text-sm leading-relaxed font-medium" style={{ color: headingColor }}>{club.whoItsFor}</p>
          </div>
